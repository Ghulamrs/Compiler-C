...
Measured by putting all 404 single-file cases through each backend and counting
...
| --- | --- | --- | --- |
| `x86_64-linux` | **404 / 404** | 0 | nothing |
| `x86_64-windows` | **403 / 404** | 1 | nothing that is a gap. The one refusal, `bf_types.c`, asks for a 40-bit field in an `unsigned long` — 32 bits under LLP64 — so refusing is correct C. |
| `arm64-darwin` | **404 / 404** | 0 | nothing |
...
All 404 agree with clang exactly, checked by compiling each twice and comparing
